...
namespace Infinite_World
{
    internal class TerrainChunk
    {
        ...
        public void LoadChunk(int mapSeed, GraphicsDevice graphics, SpriteBatch spriteBatch, List<Biome> biomes)
        {
            ...
            double[,] heightMap = noiseGenerator.GenerateNoiseMap(mapSeed, _chunkSize, _address * 100, 3, 0.06f, 4);
            double[,] heatMap = noiseGenerator.GenerateNoiseMap(mapSeed, _chunkSize, _address * 100, 3, 0.04f, 2);
            double[,] moistureMap = noiseGenerator.GenerateNoiseMap(mapSeed, _chunkSize, _address * 100, 3, 0.03f, 1);
